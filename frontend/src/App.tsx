...
export default function App() {
  const pageSize = 24
  const { data, fetchNextPage, hasNextPage, isFetchingNextPage } = useProducts({ pageSize })
            ...
            <CardContent className="space-y-3">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <Badge variant="secondary" className="gap-1">
                  <Filter className="h-3.5 w-3.5" /> Category: All
                </Badge>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="h-full flex flex-col">
            ...
            <CardContent className="flex-1 p-0">
              <ScrollArea className="h-[calc(100vh-16rem)] p-4">
                <div className="ml-auto max-w-[80%] rounded-2xl bg-blue-600 text-white px-3 py-2">Hi! Looking for anything specific?</div>
              </ScrollArea>
            </CardContent>
            <Separator />
            <div className="p-3">
              <div className="flex gap-2">
                <Input className="h-10" placeholder="Ask for recommendations..." />
